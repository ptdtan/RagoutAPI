...
class ChimeraDetector4Ancestor(object):
    def __init__(self, breakpoint_graphs, run_stages, ancestor_seqs):
        logger.debug("Detecting chimeric adjacencies")
        self.bp_graphs = breakpoint_graphs
        self.run_stages = run_stages
        self.ancestor_seqs = ancestor_seqs
    ...
    def _optimal_break(self, seq_name, break_start, break_end):
        ...
        """
        seq = self.ancestor_seqs[seq_name]
        cur_pos = break_start
        cur_len = 0
        max_pos = break_start
        max_len = 0
    ...
    def _get_contig_breaks(self, bp_graph):
        ...
        for subgr in subgraphs:
            ...
            for (u, v, data) in subgr.bp_graph.edges_iter(data=True):
                if data["genome_id"] != subgr.ancestor:
                    continue

                genomes = set(subgr.genomes_support(u, v))
                if (genomes != set([bp_graph.ancestor])
    ...
    def break_contigs(self, perm_container, block_sizes):
        ...
        for perm in new_container.ancestor_perms:
            break_points = []
            ...
            if not break_points:
                new_ancestor_perms.append(perm)
            else:
                num_chimeras += 1
                new_ancestor_perms.extend(_break_permutation(perm, break_points))

        logger.debug("Chimera Detector: {0} cuts made in {1} sequences"
                        .format(num_breaks, num_chimeras))
        new_container.ancestor_perms = new_ancestor_perms
